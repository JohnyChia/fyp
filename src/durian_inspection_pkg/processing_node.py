...
class ProcessingNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('leaf_inference_scales', [1024, 768, 512])
        ...
        self.declare_parameter("processed_root", "/home/johny/durian_ws/processed_images")
        self.tree_config = {
        ...
        self.process_service = self.create_service(
            Trigger, 'process_captured_images', self.handle_process_captured,
        ...
        self.processed_root = os.path.abspath(str(self.get_parameter('processed_root').value))
        os.makedirs(self.processed_root, exist_ok=True)
        self.debug_root = os.path.join(self.processed_root, 'debug')
        os.makedirs(self.debug_root, exist_ok=True)
        with connect(self.db_path):
            pass
        self.pipeline_fingerprint = self._pipeline_fingerprint()
        self.get_logger().info('Deferred processing node ready')
    ...
    def _pipeline_fingerprint(self):
        parts = [
            'legacy-vision-pipeline-v1',
            self._sha256_file(str(self.get_parameter('model_tree_path').value)),
            self._sha256_file(str(self.get_parameter('model_fruit_path').value)),
            self._sha256_file(str(self.get_parameter('model_leaf_path').value)),
            repr(self.get_parameter('leaf_inference_scales').value),
            repr(self.get_parameter('leaf_roi_context_scale').value),
            repr(self.get_parameter('leaf_roi_unreliable_context_scale').value),
            repr(self.get_parameter('leaf_roi_max_area_ratio').value),
            repr(self.get_parameter('leaf_roi_min_size').value),
        ]
        return hashlib.sha256('|'.join(parts).encode('utf-8')).hexdigest()

    @staticmethod
    def _transform_from_dict(data):
        if not data:
            return None
        msg = TransformStamped()
        msg.header.frame_id = str(data.get('header', {}).get('frame_id', ''))
    ...
    @staticmethod
    def _inverse_transform(msg):
        if msg is None:
            return None
        q = msg.transform.rotation
        x, y, z, w = float(q.x), float(q.y), float(q.z), float(q.w)
        norm = max(1e-12, x*x + y*y + z*z + w*w)
        ...
        inv_t = -(rotation.T @ translation)
        inverse = TransformStamped()
        inverse.header.frame_id = msg.child_frame_id
        inverse.child_frame_id = msg.header.frame_id
        inverse.transform.translation.x = float(inv_t[0])
        inverse.transform.translation.y = float(inv_t[1])
        inverse.transform.translation.z = float(inv_t[2])
        inverse.transform.rotation.x = -x
        inverse.transform.rotation.y = -y
        inverse.transform.rotation.z = -z
        inverse.transform.rotation.w = w
        return inverse

    def handle_process_captured(self, request, response):
        del request
        if not self.models_ready:
            self.load_all_models()
        if not self.models_ready:
            response.success = False
            response.message = 'Models are not ready; rows remain labelled processing'
            return response
        self.pipeline_fingerprint = self._pipeline_fingerprint()
        try:
            completed, cache_hits = self.process_pending_trees()
            response.success = True
            response.message = f'completed={completed}, cache_hits={cache_hits}'
        except Exception as exc:
            self.get_logger().error(f'Deferred processing failed: {exc}\n{traceback.format_exc()}')
            response.success = False
            response.message = str(exc)
        return response

    def process_pending_trees(self):
        with connect(self.db_path) as conn:
            pending = conn.execute(
                """SELECT inspection_id, tree_id, tree_x, tree_y
                   FROM inspection_log WHERE status='processing'
                   ORDER BY id"""
            ).fetchall()
        completed = 0
        cache_hits = 0
        for inspection_id, tree_id, tree_x, tree_y in pending:
            try:
                was_cached = self._process_tree_observations(
                    str(inspection_id), int(tree_id), float(tree_x), float(tree_y)
                )
                completed += 1
                cache_hits += int(was_cached)
            except Exception as exc:
                self.get_logger().error(
                    f'Processing retained as processing: inspection={inspection_id} '
                    f'tree={tree_id}: {exc}'
                )
                with connect(self.db_path) as conn:
                    conn.execute(
                        """UPDATE inspection_log SET processing_error=?
                           WHERE inspection_id=? AND tree_id=? AND status='processing'""",
                        (str(exc), inspection_id, tree_id),
                    )
        return completed, cache_hits

    def _process_tree_observations(self, inspection_id, tree_id, tree_x, tree_y):
        with connect(self.db_path) as conn:
            rows = conn.execute(
                """SELECT observation_id, rgb_path, depth_path, metadata_path,
                          observation_sha256, rgb_sha256, depth_sha256,
                          context_sha256
                   FROM captured_observations
                   WHERE inspection_id=? AND tree_id=?
                   ORDER BY rgb_timestamp, id""",
                (inspection_id, tree_id),
            ).fetchall()
        if not rows:
            raise RuntimeError('no captured observations found')
        ...
        duplicates = []
        for row in rows:
            context_hash = row[7] or self._sha256_file(row[3])
            signature = (row[4], context_hash)
            canonical = canonical_by_signature.get(signature)
            if canonical is None:
                canonical_by_signature[signature] = row[0]
                unique_rows.append(row)
            else:
                duplicates.append((row[0], canonical))
        if duplicates:
            with connect(self.db_path) as conn:
                conn.executemany(
                    """UPDATE captured_observations
                       SET status='DUPLICATE', duplicate_of=?, processed_at=CURRENT_TIMESTAMP
                       WHERE observation_id=?""",
                    [(canonical, duplicate) for duplicate, canonical in duplicates],
                )
        rows = unique_rows

        batch_material = '|'.join(f'{row[4]}:{row[7] or self._sha256_file(row[3])}' for row in rows)
        batch_material += f'|target={tree_x:.9f},{tree_y:.9f}'
        batch_hash = hashlib.sha256(batch_material.encode('utf-8')).hexdigest()
        with connect(self.db_path) as conn:
            cached = conn.execute(
                """SELECT source_inspection_id, disease_name, coverage, confidence,
                          priority, remedy
                   FROM tree_processing_cache
                   WHERE batch_sha256=? AND pipeline_fingerprint=?""",
                (batch_hash, self.pipeline_fingerprint),
            ).fetchone()
        if cached:
            source, disease, coverage, confidence, priority, remedy = cached
            self._complete_tree(
                inspection_id, tree_id, disease, coverage, confidence,
                priority, remedy, True, source
            )
            with connect(self.db_path) as conn:
                conn.execute(
                    """UPDATE captured_observations
                       SET status='CACHE_HIT', processed_at=CURRENT_TIMESTAMP,
                           processing_error=NULL
                       WHERE inspection_id=? AND tree_id=? AND status!='DUPLICATE'""",
                    (inspection_id, tree_id),
                )
            return True
        ...
        with self.state_lock:
            ...
            self.scan_target_tree_id = tree_id
            self.scan_target_map_x = tree_x
            self.scan_target_map_y = tree_y
            self.tree_roi_cache.pop(tree_id, None)
            self.scan_target_lock = {
                'epoch': epoch, 'target_id': tree_id, 'tracking_id': None,
                'real_x': None, 'real_y': None, 'real_z': None,
                'conf': 0.0, 'last_seen': 0.0,
            }
            self.scan_disease_votes[(epoch, tree_id)] = {
                'epoch': epoch, 'target_id': tree_id, 'counts': {}, 'conf_sum': {},
                'total_crops': 0, 'yolo_frames': 0, 'valid_frames': 0,
                'rejected_frames': 0, 'sum_raw_conf': 0.0,
                'max_raw_conf': 0.0, 'max_valid_conf': 0.0,
            }

        output_dir = os.path.join(self.processed_root, inspection_id, str(tree_id))
        os.makedirs(output_dir, exist_ok=True)
        for observation_id, rgb_path, depth_path, metadata_path, _, rgb_sha, depth_sha, _ in rows:
            with connect(self.db_path) as conn:
                conn.execute(
                    "UPDATE captured_observations SET status='PROCESSING', processing_error=NULL WHERE observation_id=?",
                    (observation_id,),
                )
            try:
                ...
                if frame is None or depth is None:
                    raise IOError('captured RGB/depth file cannot be decoded')
                if hashlib.sha256(np.ascontiguousarray(frame).tobytes()).hexdigest() != rgb_sha:
                    raise IOError('RGB SHA-256 mismatch')
                if hashlib.sha256(np.ascontiguousarray(depth).tobytes()).hexdigest() != depth_sha:
                    raise IOError('depth SHA-256 mismatch')
                with open(metadata_path, encoding='utf-8') as stream:
                    metadata = json.load(stream)
                self._load_observation_context(metadata, depth)
                stamp_ns = int(float(metadata['rgb']['timestamp']) * 1_000_000_000)
                stamp = rclpy.time.Time(nanoseconds=stamp_ns).to_msg()
                transforms = metadata.get('transforms', {})
                odom_from_camera = self._transform_from_dict(transforms.get('odom_from_camera'))
                frame_context = {
                    'stamp': stamp, 'scan_active': True, 'target_id': tree_id,
                    'epoch': epoch, 'map_x': tree_x, 'map_y': tree_y,
                    'odom_from_camera': odom_from_camera,
                    'camera_from_odom': self._inverse_transform(odom_from_camera),
                    'odom_from_map': self._transform_from_dict(transforms.get('odom_from_map')),
                }
                self._offline_camera_from_odom = frame_context['camera_from_odom']
                self._offline_odom_from_map = frame_context['odom_from_map']
                with self.gpu_lock:
                    self.detect_and_publish_trees(frame, frame_context)
                processed_path = os.path.join(output_dir, f'{observation_id}_processed.jpg')
                if not cv2.imwrite(processed_path, frame):
                    raise IOError(f'cannot write {processed_path}')
                with connect(self.db_path) as conn:
                    conn.execute(
                        """UPDATE captured_observations SET status='PROCESSED',
                                  processed_at=CURRENT_TIMESTAMP, processing_error=NULL
                           WHERE observation_id=?""",
                        (observation_id,),
                    )
            except Exception as exc:
                with connect(self.db_path) as conn:
                    conn.execute(
                        "UPDATE captured_observations SET status='FAILED', processing_error=? WHERE observation_id=?",
                        (str(exc), observation_id),
                    )
                raise

        with self.state_lock:
            evidence = dict(self.scan_disease_votes.get((epoch, tree_id), {}))
            evidence['counts'] = dict(evidence.get('counts', {}))
            evidence['conf_sum'] = dict(evidence.get('conf_sum', {}))
        request = TreeStatus.Request()
        request.tree_id = str(tree_id)
        result = self.handle_get_status(request, TreeStatus.Response())
        self._complete_tree(
            inspection_id, tree_id, result.disease_status, result.coverage,
            result.confidence, result.priority, result.remedy, False, None
        )
        with connect(self.db_path) as conn:
            conn.execute(
                """INSERT OR REPLACE INTO tree_processing_cache (
                    batch_sha256, pipeline_fingerprint, source_inspection_id,
                    source_tree_id, disease_name, coverage, confidence, priority,
                    remedy, evidence_json
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (batch_hash, self.pipeline_fingerprint, inspection_id, tree_id,
                 result.disease_status, result.coverage, result.confidence,
                 result.priority, result.remedy, json.dumps(evidence, sort_keys=True)),
            )
        return False

    def _load_observation_context(self, metadata, depth):
        info = metadata['camera_info']
        msg = CameraInfo()
        msg.header.frame_id = str(info.get('frame_id', 'camera_link_optical'))
        msg.width = int(info['width'])
        msg.height = int(info['height'])
        msg.distortion_model = str(info.get('distortion_model', ''))
        msg.d = [float(v) for v in info.get('d', [])]
        msg.k = [float(v) for v in info['k']]
        msg.r = [float(v) for v in info.get('r', [0.0] * 9)]
        msg.p = [float(v) for v in info.get('p', [0.0] * 12)]
        self.camera_info_msg = msg
        depth_t = float(metadata['depth']['timestamp'])
        depth_stamp = rclpy.time.Time(nanoseconds=int(depth_t * 1_000_000_000)).to_msg()
        with self.state_lock:
            self.depth_buffer.clear()
            self.depth_buffer.append((depth_t, depth_stamp, depth))

    def _complete_tree(self, inspection_id, tree_id, disease, coverage, confidence,
                       priority, remedy, cache_hit, cache_source):
        with connect(self.db_path) as conn:
            conn.execute(
                """UPDATE inspection_log SET status='completed', disease_name=?,
                          coverage=?, confidence=?, priority=?, remedy=?,
                          processing_error=NULL, cache_hit=?,
                          cache_source_inspection_id=?, completed_at=CURRENT_TIMESTAMP
                   WHERE inspection_id=? AND tree_id=? AND status='processing'""",
                (disease, coverage, confidence, priority, remedy, int(cache_hit),
                 cache_source, inspection_id, tree_id),
            )
    ...
    def detect_and_publish_trees(self, frame, frame_context):
        ...
        try:
            ...
            trans = frame_context.get('odom_from_camera')
            if trans is None:
                trans = self.tf_buffer.lookup_transform(
                    "odom", "camera_link_optical", stamp
                )
        ...
        tree_debug_input = os.path.join(self.debug_root, "tree_yolo_input.jpg")
        cv2.imwrite(tree_debug_input, frame)
        self.log_diagnostic(f"[TREE YOLO DEBUG IMAGE]\npath={tree_debug_input}\nshape={frame.shape}")
        ...
        with torch.no_grad():
            ...
            if not frame_scan_active or frame_target_id is None:
                ...
                odom_target_z = 1.2
                try:
                    trans = frame_context.get('odom_from_map')
                    if trans is None:
                ...
                if fresh_target is None and frame_map_x is not None and frame_map_y is not None:
                    try:
                        ...
                        map_to_odom = frame_context.get('odom_from_map')
                        ...
                        odom_to_camera = frame_context.get('camera_from_odom')
                        if odom_to_camera is None:
    ...
    def handle_get_status(self, request, response):
        ...
        import os
        debug_dir = os.path.join(self.debug_root, "disease_reports")
    ...
    def make_leaf_crop(self, frame, bbox, real_x=None, real_y=None, real_z=None, stamp=None):
        ...
        if projection_eligible:
            try:
                ...
                trans = getattr(self, '_offline_camera_from_odom', None)
                if trans is None:
                    trans = self.tf_buffer.lookup_transform(
                        "camera_link_optical", "odom", stamp
                    )
    ...
    def process_tree(self, frame, box, tracking_id, vote_target_id, marker_array, stamp, real_x, real_y, real_z, tree_conf, frame_epoch=None):
        ...
        if self.model_leaf:
            ...
            if crop is None:
                ...
                import cv2
                saved = cv2.imwrite(os.path.join(self.debug_root, f"tree_{tracking_id}_{debug_id}.jpg"), crop)
                if not saved:
    ...
    def detect_leaf_disease(self, crop, tracking_id, vote_target_id, debug_id, tree_conf, frame_epoch=None):
        ...
        if crop.size > 0 and self.model_leaf:
                ...
                if True:
                    ...
                    if tile_crop.size > 0:
                        ...
                        for leaf_imgsz, leaf_result, box in leaf_detection_records:
                            ...
                            dynamic_area_limit = 0.85 - (0.40 * scale_factor)
                            dynamic_size_limit = 0.95 - (0.15 * scale_factor)
            ...
            cv2.imwrite(os.path.join(self.debug_root, f"leaf_{matched_id}_{debug_id}.jpg"), debug_crop)
