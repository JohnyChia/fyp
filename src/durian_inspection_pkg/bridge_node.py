...
class BridgeNode(Node):
    def __init__(self):
        ...
        self.publish_camera = os.environ.get('DURIAN_BRIDGE_PUBLISH_CAMERA', 'false').lower() == 'true'
        self.pub_image = self.create_publisher(Image, '/camera/image_raw', 10) if self.publish_camera else None
        self.pub_info = self.create_publisher(CameraInfo, '/camera/camera_info', 10) if self.publish_camera else None
        self.get_logger().info(f'Bridge camera publishing: {self.publish_camera}')
