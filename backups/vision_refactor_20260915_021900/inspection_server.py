...
class InspectionServer(Node):
    def __init__(self):
        ...
        # 修复：使用与 map_publisher.py 一致的 TRANSIENT_LOCAL QoS
        ...
        # Stable tree identity comes from the active Gazebo world.
        ...
        self.goal_pose = None
        self._cached_tree_res = None  # explicit init; reset per-tree at CALCULATE_GOAL

        self.load_completed()

        # 启动时从 durian_data.db 预加载树木位置（冷启动备用方案）
    ...
    def init_database(self):
        try:
            ...
            """)
            try: cursor.execute("ALTER TABLE inspection_log ADD COLUMN disease_name TEXT")
            except: pass
            try: cursor.execute("ALTER TABLE inspection_log ADD COLUMN coverage REAL")
            except: pass
            try: cursor.execute("ALTER TABLE inspection_log ADD COLUMN priority TEXT")
            except: pass
            try: cursor.execute("ALTER TABLE inspection_log ADD COLUMN remedy TEXT")
            except: pass
            
            # Safe migration for confidence
            cursor.execute("PRAGMA table_info(inspection_log)")
            columns = [info[1] for info in cursor.fetchall()]
            if 'confidence' not in columns:
                cursor.execute("ALTER TABLE inspection_log ADD COLUMN confidence REAL")
                ...
                for lx, ly, lz, _ in local_trees:
                    ...
                    for i, (tx, ty, tz, tid) in enumerate(self.trees):
                        if math.hypot(lx - tx, ly - ty) < self.TREE_MATCH_RADIUS:
                            # Use Exponential Moving Average to gently track the drifted position
                            # Do NOT drift the currently locked tree target
                            is_locked = False
    ...
    def load_trees_from_db(self):
        ...
        try:
            ...
            cursor = conn.cursor()
            # rowid%20 采样（比 %50 密集，保证稀疏区域的树有足够聚类点）
            cursor.execute(
            ...
            # 宽松 DBSCAN 参数：eps=0.8（适配稀疏采样点间距），min_samples=3（稀疏时也能聚类）
    ...
    def find_nearest_tree(self):
        ...
        # 修复：使用 map 坐标系下的机器人位置，避免 odom 漂移导致距离计算错误
        pose_tuple = self.get_robot_pose_in_map()
        if pose_tuple is None or pose_tuple[0] is None:
        ...
        # Target Locking: if already navigating to a tree, keep it unless it's completed
        if self.current_tree and self.state == "NAVIGATING":
            ...
            if candidates:
                # Stable sort: primary by distance, secondary by tree_id
            ...
            if current_state == "FIND_TREE":
                ...
                with self.data_lock:
                    if tree == "TF_NOT_READY":
                        ...
                        if len(self.trees) == 0:
                            # Wait for tree discovery to happen via point clouds or DB
                            time.sleep(0.5)
                        else:
                ...
                if not nav_active and nav_result is None:
                    # START SMOOTHNESS: Ensure robot is stationary before sending goal
                ...
                actual_dist = math.hypot(tx - rx, ty - ry)
                if actual_dist < 1.0: # MINIMUM_SAFE_DISTANCE
                ...
                # Ensure no cmd_vel is published during scanning
                self.stop_robot()
                
                # Keep TF timestamp fresh to prevent false stale aborts
                self.get_robot_pose_in_map()
                
                if time.time() - getattr(self, '_last_tf_success_time', 0.0) > 2.0:
                    self.get_logger().error("TF STALE for > 2.0s during SCANNING! Aborting.")
                    _abort_tid = self.scan_debug_start.get('tid', None) if hasattr(self, 'scan_debug_start') else None
                    if _abort_tid is not None:
                        _res = self._fetch_tree_status(_abort_tid)
                        if _res is not None:
                            self._cached_tree_res = _res
                    _sa_msg = Bool()
                    _sa_msg.data = False
                ...
                # Wait for 10 seconds stationary scan
                if scan_time_elapsed > 10.0:
                    self.get_logger().info("SCAN_COMPLETE: 10 seconds stationary scan finished.")
                    
                    _tid = self.scan_debug_start.get('tid', None)
                    if _tid is not None:
                        _res = self._fetch_tree_status(_tid)
                        if _res is not None:
                            self._cached_tree_res = _res
                ...
                if tree:
                    ...
                    # nav_retry is a navigation recovery counter, not distance.
                    # Store the actual map-frame robot-to-target distance.
                    if (
                    ...
                    if hasattr(self, '_cached_tree_res') and self._cached_tree_res:
                        res = self._cached_tree_res
                        disease_name = res.disease_status
                    ...
                    try:
                        ...
                        import datetime
                        cursor.execute(
                            "INSERT INTO inspection_log (tree_x, tree_y, distance, scan_time, status, timestamp, disease_name, coverage, confidence, priority, remedy) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                            (tx, ty, inspection_distance, float(scan_duration), "completed", datetime.datetime.now(), disease_name, coverage, confidence, priority, remedy)
                        )
                        conn.commit()
                        conn.close()
                        self.get_logger().info("TREE MARKED INSPECTED")
                    except Exception as e:
    ...
    def _fetch_tree_status(self, tid: int):
        ...
        """
        try:
            # We create a temporary node to call the service safely from this thread 
            # without deadlocking or waiting on the main executor threads which might be busy.
